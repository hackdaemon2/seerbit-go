...
func ProcessUpdateOrderDemo() {
	...
	orderPayload := createUpdateOrderPayload(seerBitClient.PublicKey)

	response, err := order.Update(orderPayload)
	if err != nil {
...
func createUpdateOrderPayload(publicKey string) model.OrderPostPaymentPayload {
